import torch
import torch.optim as optim
from tqdm import tqdm
from .base_trainer import BaseTrainer
from utils.metrics import evaluate_ranking


class SASRecTrainer(BaseTrainer):
    """Trainer for SASRec with BPR loss and linear LR warmup."""

    def __init__(self, model, config, device):
        super().__init__(model, config, device)
        self.optimizer = optim.Adam(
            model.parameters(),
            lr=config.get("lr", 1e-3),
            betas=(0.9, 0.98),
            weight_decay=config.get("weight_decay", 1e-5),
        )
        warmup_steps = config.get("warmup_steps", 100)
        self.scheduler = optim.lr_scheduler.LambdaLR(
            self.optimizer,
            lr_lambda=lambda step: min(1.0, step / max(warmup_steps, 1)),
        )
        self.max_seq_len = config.get("max_seq_len", 50)
        self._step = 0

    def train_epoch(self, loader) -> float:
        self.model.train()
        total_loss = 0.0
        for batch in tqdm(loader, desc="SASRec train", leave=False):
            seq      = batch["seq"].to(self.device)
            seq_len  = batch["seq_len"].to(self.device)
            pos_item = batch["pos_item"].to(self.device)
            neg_item = batch["neg_item"].to(self.device)

            self.optimizer.zero_grad()
            loss = self.model(seq=seq, pos_item=pos_item, neg_item=neg_item, seq_len=seq_len)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(self.model.parameters(), 5.0)
            self.optimizer.step()
            self.scheduler.step()
            self._step += 1
            total_loss += loss.item()

        return total_loss / len(loader)

    def evaluate(self, eval_data, user_sequences, n_items):
        return evaluate_ranking(
            model=self.model,
            eval_data=eval_data,
            user_sequences=user_sequences,
            n_items=n_items,
            device=self.device,
            k_list=self.config.get("k_list", [5, 10, 20]),
            n_neg_candidates=self.config.get("n_neg_candidates", 100),
            model_type="sasrec",
            max_seq_len=self.max_seq_len,
        )